...
class Services_Soundcloud
{
     ...
     */
     ...
     //AN - takes $ params and adds $default params to it. makes url params array and 'authorize' from 
     //$_paths array l.161 using buildUrl()l.787
     // $params set to $defaultParams + $params passed into function parameters
     ...
     *
     * @access public
     * @see Soundcloud::_getAccessToken() an - l822
	 * an - takes Sc client details as $defaultPostData. Takes $curlOptions. 
     */
     ...
     */
     ...
     /*AN - returns a url 
	  * built from either preg match $path(if is url with http etc) or makes url string with https etc,
     either 'connect' or 'api', development/production domain name(?), version of api and $path. 
	 AND Then adds either count($params)(counts elements in an array.outputs a number) or http_build_query($params) to $url before returning */
     ...
     *
     * @access protected
	 * an - returns json_decode(json to ass. array) with getAccessTokenUrl and $postdata+$curloptions
	 *  inputed into request()
     */
